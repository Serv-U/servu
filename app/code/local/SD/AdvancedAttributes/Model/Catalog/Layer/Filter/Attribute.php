<?php
class SD_AdvancedAttributes_Model_Catalog_Layer_Filter_Attribute extends Mage_Catalog_Model_Layer_Filter_Attribute
{
    ...
    public function getGroupName($setId = 4) { 
        ...
        if($setId != 0) {
            $attribute = $this->getAttributeModel();
            $group_id = $attribute->getData('attribute_set_info/' . $attribute->getAttributeSetId()  . '/group_id');
            $group = Mage::getModel('eav/entity_attribute_group')->load($group_id);
            $group_name = $group->getData('attribute_group_name');
        }
