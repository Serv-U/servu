 ...
 */
class ShoppingFeeds_Feed_Block_Adminhtml_Thefind_Edit_Codes_Edit_Form extends Mage_Adminhtml_Block_Widget_Form
{
    /**
     * Import codes list
     *
    ...
    {
        $attributes = Mage::getConfig()->getNode(ShoppingFeeds_Feed_Model_Thefindimport::XML_NODE_FIND_FEED_ATTRIBUTES)->children();
        $result     = array();
    ...
    protected function _prepareForm()
    {
        ...
        $fieldset = $form->addFieldset('generate_fieldset', array(
            'legend' => Mage::helper('shoppingfeeds_feed')->__('Item params')
        ));
        $fieldset->addField('import_code', 'select', array(
            'label'     => Mage::helper('shoppingfeeds_feed')->__('TheFind\'s Import code'),
            'name'      => 'import_code',
            'required'  => true,
            'options'   => $this->_getImportCodeList()
        ));
        $fieldset->addField('eav_code', 'select', array(
            'label'     => Mage::helper('shoppingfeeds_feed')->__('Magento\'s Eav code'),
            'name'      => 'eav_code',
            'required'  => true,
        ...
        $isEnabledOptions = $source->getOptionArray();

        $fieldset->addField('enabled', 'select', array(
            'label'     => Mage::helper('shoppingfeeds_feed')->__('Enabled'),
            'name'      => 'enabled',
            'value'     => 1,
