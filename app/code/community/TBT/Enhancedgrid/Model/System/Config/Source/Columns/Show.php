...
class TBT_Enhancedgrid_Model_System_Config_Source_Columns_Show
{
    public function toOptionArray()
    {
        ...
        foreach($collection->getItems() as $col) {
            //DM added attribute code to the label
            $cols[] = array('value' => $col->getAttributeCode(),   'label' => $col->getFrontendLabel().' - '.$col->getAttributeCode());;
        }
        return $cols;
    }
    ...
    protected function _getAttrCol() {
        ...
        //DM added order by label
        $collection->addFilter( "is_visible", 1 )->setOrder('frontend_label', 'ASC');
